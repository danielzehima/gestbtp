...
def abonnes():
    ...
    total_revenu = db.session.query(func.sum(User.revenu_genere)).filter_by(role=RoleEnum.CLIENT).scalar() or 0
    stats = {
        'total_clients': User.query.filter_by(role=RoleEnum.CLIENT).count(),
        'starter': User.query.filter_by(role=RoleEnum.CLIENT, plan=PlanEnum.STARTER).count(),
        'pro': User.query.filter_by(role=RoleEnum.CLIENT, plan=PlanEnum.PRO).count(),
        'entreprise': User.query.filter_by(role=RoleEnum.CLIENT, plan=PlanEnum.ENTREPRISE).count(),
